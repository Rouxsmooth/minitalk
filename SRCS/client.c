...
/*   Updated: 2024/12/26 23:34:16 by mzaian           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float get_usleep(size_t messlen)
{
	...
	return ((2000 / divisor) + (1.5 * messlen) + 300);
}
...
void	main(int argc, char **argv)
{
	...
	usleep_time = get_usleep(ft_strlen(argv[1]));
	ft_printf("usleep time = %d pour messlen de %d\n", (int) usleep_time, (int) ft_strlen(argv[1]));
	while (*argv[1])
	{
		i = 7;
		while (i >= 0)
